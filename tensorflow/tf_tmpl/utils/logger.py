...
class DefinedSummarizer:
    def __init__(self, sess, config, scalar_tags=None, images_tags=None):
        """
        ...
        self.summary_writer = tf.summary.FileWriter(config.summary_dir)

        """
        # jwjang: crusader76@icloud.com
        if "comet_api_key" in config:
            from comet_ml import Experiment
            self.experiment = Experiment(api_key=self.config['comet_api_key'], project_name=self.config['exp_name'])
            self.experiment.log_multiple_params(self.config)
        """
...
class Logger:
    def __init__(self, sess,config):
        ...
        self.train_summary_writer = tf.summary.FileWriter(\
                                    os.path.join(self.config.summary_dir,
