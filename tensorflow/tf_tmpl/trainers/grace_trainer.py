...
class GraceTrainer(BaseTrain):
    def __init__(self, sess, model, config, logger, data_loader):
        """
        Constructing this trainer based on the Base Train..
        ...
        :param data_loader:
        """
        super(GraceTrainer, self).__init__(sess, model, config, logger,
                                           data_loader)
        ...
        self.train_op, self.loss_node, self.acc_node = \
    ...
    def train(self):
        ...
        :return:
        """
        print("Traning started. It takes sometime...")
    ...
    def train_epoch(self, epoch=None):
        ...
        self.summarizer.summarize(self.model.global_step_tensor.eval(self.sess),
                                  summaries_dict)

        self.model.save(self.sess)
        
        print("loss:{:.4f} -- acc:{:.4f}".format(loss_per_epoch.val,
                                                 acc_per_epoch.val))
    ...
    def train_step(self):
        ...
        """
        _, loss, acc = self.sess.run([self.train_op, self.loss_node,
                                      self.acc_node],
