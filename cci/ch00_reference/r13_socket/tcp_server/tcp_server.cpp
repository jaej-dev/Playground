/*
 * tcp_server.cpp: multi thread test server
 * Compile: g++ tcp_server.cpp -o svr -pthread
 */
    ...
    for (int y=7 ; y>-1 ; y--)
    {
        ...
        for (int x=0 ; x<8 ; x++)
        {
            ...
            if (chessboard[x][y] == "X") // X represents an empty space.
            {
                if (isblack(x+1,y+1) )
                {
                    chessboard[x][y] = clr_black_square + "▩" ; // unicode char for a black square
                }
                else
                    chessboard[x][y] = clr_white_square + "▢" ; // unicode char for a white square
            }
...
bool movepiece(char * move)  // Function that executes moves
{
    ...
    if (chessboard [a][b] =="\033[01;34m▩" || chessboard [a][b] =="\033[01;36m▢")
    {
        accept = false ;
    }
    else
    {
        ...
        if (chessboard[c][d] != "X" && chessboard[c][d] != clr_black_square+"▩"  && chessboard[c][d] != clr_white_square +"▢") killed.insert(killed.begin() ,chessboard[c][d] ) ;
...
void newgame()  // Function to set the chessboard to it's begin status.
{
    ...
    chessboard[7][7] = clr_black + "♜";
    chessboard[6][7] = clr_black + "♞";
    chessboard[5][7] = clr_black + "♝";
    chessboard[4][7] = clr_black + "♚";
    chessboard[3][7] = clr_black + "♛";
    chessboard[2][7] = clr_black + "♝";
    chessboard[1][7] = clr_black + "♞";
    chessboard[0][7] = clr_black + "♜";
    for (int x=0; x<8 ; x++)
        chessboard[x][6] = clr_black + "♟";


    chessboard[7][0] = clr_white +"♖";
    chessboard[6][0] = clr_white +"♘";
    chessboard[5][0] = clr_white +"♗";
    chessboard[4][0] = clr_white +"♔";
    chessboard[3][0] = clr_white +"♕";
    chessboard[2][0] = clr_white +"♗";
    chessboard[1][0] = clr_white +"♘";
    chessboard[0][0] = clr_white +"♖";
    for (int x=0; x<8 ; x++)
        chessboard[x][1] = clr_white +"♙";
